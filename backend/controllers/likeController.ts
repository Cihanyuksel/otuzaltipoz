...
const getPhotoLikes = async (
  ...
  try {
    ...
    const likes = await Like.find({ photo_id: photoId }).populate({
      path: "user_id",
      select: "username profile_img_url role",
    });
    ...
    res.status(200).json({
      message: "Fotoğrafın beğeni bilgileri başarıyla getirildi.",
      success: true,
      photoId,
      likeCount,
      isLikedByMe,
