...
export const maxRefreshTokenRotations = createLimiter({
  ...
  max: 30,
  message:
    "Çok fazla token yenileme denemesi yaptınız. Lütfen biraz sonra tekrar deneyin.",
});
...
export const maxForgetPasswordRotations = createLimiter({
  ...
  max: 3,
  message:
    "Bu işlem için çok fazla deneme yaptınız. Lütfen bir saat sonra tekrar deneyin.",
});
...
export const maxContactRotations = createLimiter({
  ...
  max: 5,
  message:
    "Günlük iletişim mesajı limitine ulaşıldı. Lütfen yarın tekrar deneyin.",
});
...
export const aiAnalysisLimiter = rateLimit({
  ...
  message: {
    status: "fail",
    message: "Çok fazla analiz isteği gönderdiniz. Lütfen 1 dakika bekleyin.",
  },
  standardHeaders: true,
  legacyHeaders: false,
});
